...
export type AnimationType = 'four' | 'six' | 'wicket';
...
export function SixAnimation({ onComplete }: { onComplete: () => void }) {
  useEffect(() => {
...
export function FourAnimation({ onComplete }: { onComplete: () => void }) {
  useEffect(() => {
...
export function WicketAnimation({ onComplete }: { onComplete: () => void }) {
  useEffect(() => {
    const timer = setTimeout(onComplete, 2000);
    return () => clearTimeout(timer);
  }, [onComplete]);
...
interface ScoringAnimationsProps {
  type: AnimationType | null;
  onComplete: () => void;
}

export function ScoringAnimations({ type, onComplete }: ScoringAnimationsProps) {
  return (
    <AnimatePresence>
      {type === 'six' && <SixAnimation onComplete={onComplete} />}
      {type === 'four' && <FourAnimation onComplete={onComplete} />}
      {type === 'wicket' && <WicketAnimation onComplete={onComplete} />}
    </AnimatePresence>
  );
}
